...
const useChatStore = create((set, get) => ({
  ...
  setActiveChat(chatId) {
    ...
    const unsub = onSnapshot(q, snap => {
      set(s => ({ messages: { ...s.messages, [chatId]: snap.docs.map(d => ({ id: d.id, ...d.data() })) } }))
    })
  ...
  async sendMessage(chatId, payload) {
    ...
    await updateDoc(doc(db, 'chats', chatId), {
      lastMessage: payload.text ? (payload.text.length > 60 ? payload.text.slice(0,60)+'…' : payload.text) : '📎 Attachment',
