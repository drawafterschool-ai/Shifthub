...
// Declare the secrets we need — this makes process.env.SMTP_USER / SMTP_PASS available
const EMAIL_SECRETS = ['SMTP_USER', 'SMTP_PASS']

// ─────────────────────────────────────────────────────────────────────────────
// NODEMAILER TRANSPORT — created once per cold start
// ─────────────────────────────────────────────────────────────────────────────
function makeTransport() {
  const user = (process.env.SMTP_USER || '').trim()
  const pass = (process.env.SMTP_PASS || '').replace(/\s/g, '')  // strip any spaces from app password
  if (!user || !pass) {
    console.warn('SMTP_USER or SMTP_PASS not configured')
    return null
  }
  ...
  return require('nodemailer').createTransport({
    host:   'smtp.gmail.com',
    port:   465,
    secure: true,                    // SSL on port 465 — more reliable from Cloud Functions
    auth:   { user, pass },
...
// ─────────────────────────────────────────────────────────────────────────────
// SHARED DELIVERY — email + FCM push + SMS
// ─────────────────────────────────────────────────────────────────────────────
async function deliver(user, subject, body) {
  ...
  if (user.fcmToken) {
    try {
      ...
      if (e.code === 'messaging/registration-token-not-registered') {
        await db.collection('users').doc(user.id).update({ fcmToken: null }).catch(() => {})
      }
    }
  }
  ...
  if (transport && user.email) {
    ...
    } catch (e) { console.error('Email send failed:', e.message, e.code) }
  }
...
// ─────────────────────────────────────────────────────────────────────────────
// 1. DELETE AUTH WHEN USER DOC DELETED
// ─────────────────────────────────────────────────────────────────────────────
exports.onUserDeleted = onDocumentDeleted('users/{userId}', async (event) => {
  try { await admin.auth().deleteUser(event.params.userId) } catch (e) {
    if (e.code !== 'auth/user-not-found') throw e
  }
})

// ─────────────────────────────────────────────────────────────────────────────
// 2. DELIVER NOTIFICATION — email + push on each new notification doc
// ─────────────────────────────────────────────────────────────────────────────
exports.onNotificationCreated = onDocumentCreated(
  { document: 'notifications/{notifId}', secrets: EMAIL_SECRETS },
  async (event) => {
...
// ─────────────────────────────────────────────────────────────────────────────
// 3. SHIFT CHANGES — notify assigned teacher
// ─────────────────────────────────────────────────────────────────────────────
exports.onShiftChanged = onDocumentWritten(
  { document: 'shifts/{shiftId}', secrets: EMAIL_SECRETS },
  async (event) => {
    ...
    if (!after) {
      ...
      await deliver(user,
        `Shift cancelled — ${before.title || 'Your shift'}`,
    ...
    // Edited
    if (before?.instructorId && after.instructorId === before.instructorId) {
...
// ─────────────────────────────────────────────────────────────────────────────
// 4. OPEN SHIFT — notify all teachers
// ─────────────────────────────────────────────────────────────────────────────
exports.onOpenShift = onDocumentWritten(
  { document: 'shifts/{shiftId}', secrets: EMAIL_SECRETS },
  async (event) => {
    const before = event.data.before?.data()
    const after  = event.data.after?.data()
    if (!after || !after.claimable || before?.claimable) return
    const teachers = await getAllTeachers()
    for (const t of teachers) {
      await deliver(t,
        `⚡ Open shift available — ${after.date}`,
        `Hi ${t.firstName || 'there'},\n\nAn open shift is available:\n\n  📅 ${after.date}\n  🕐 ${after.start} – ${after.end}\n  📚 ${after.title || 'Open Shift'}${after.address ? `\n  📍 ${after.address}` : ''}\n\nOpen ShiftHub to claim it.\n\nhttps://yrshifts.web.app/app`
      )
    }
  }
)
...
exports.onEventCreated = onDocumentCreated(
  { document: 'events/{eventId}', secrets: EMAIL_SECRETS },
  async (event) => {
...
// ─────────────────────────────────────────────────────────────────────────────
// 6. NEW WEEKLY BUZZ — notify all teachers
// ─────────────────────────────────────────────────────────────────────────────
exports.onBuzzPostCreated = onDocumentCreated(
  { document: 'weekly_buzz/{postId}', secrets: EMAIL_SECRETS },
  async (event) => {
...
exports.sendDayBeforeReminders = onSchedule(
  ...
  async () => {
    ...
    const snap = await db.collection('shifts').where('date','==',tomorrowKey).where('status','==','published').get()
    let sent = 0
    for (const shiftDoc of snap.docs) {
      const shift = shiftDoc.data()
      if (!shift.instructorId || shift.claimable) continue
      const ref = db.collection('reminders').doc(`${shift.id}-24h`)
      if ((await ref.get()).exists) continue
      const uSnap = await db.collection('users').doc(shift.instructorId).get()
      if (!uSnap.exists) continue
      const user = { id: shift.instructorId, ...uSnap.data() }
      await deliver(user, `⏰ Reminder: shift tomorrow — ${shift.title || 'Your shift'}`, buildReminderBody(user, shift, '24h'))
      await ref.set({ shiftId: shift.id, instructorId: shift.instructorId, type: '24h', sentAt: admin.firestore.FieldValue.serverTimestamp() })
      sent++
    }
...
exports.sendTwoHourReminders = onSchedule(
  ...
  async () => {
    ...
    const central  = new Intl.DateTimeFormat('en-US', { timeZone: 'America/Chicago', hour: 'numeric', minute: 'numeric', hour12: false }).format(now)
    const [hStr, mStr] = central.split(':')
    const nowMins  = parseInt(hStr) * 60 + parseInt(mStr)
    const snap = await db.collection('shifts').where('date','==',todayKey).where('status','==','published').get()
    let sent = 0
    for (const shiftDoc of snap.docs) {
      const shift = shiftDoc.data()
      if (!shift.instructorId || shift.claimable) continue
      const startMins = parseShiftTime(shift.start)
      if (startMins < 0 || startMins < nowMins + 115 || startMins > nowMins + 150) continue
      const ref = db.collection('reminders').doc(`${shift.id}-2h`)
      if ((await ref.get()).exists) continue
      const uSnap = await db.collection('users').doc(shift.instructorId).get()
      if (!uSnap.exists) continue
      const user = { id: shift.instructorId, ...uSnap.data() }
      await deliver(user, `⏰ Starting in 2 hours — ${shift.title || 'Your shift'}`, buildReminderBody(user, shift, '2h'))
      await ref.set({ shiftId: shift.id, instructorId: shift.instructorId, type: '2h', sentAt: admin.firestore.FieldValue.serverTimestamp() })
      sent++
    }
...
// ─────────────────────────────────────────────────────────────────────────────
// 9. CREATE TEACHER ACCOUNT — sends invite email directly
// ─────────────────────────────────────────────────────────────────────────────
exports.createTeacherAccount = onCall(
  { enforceAppCheck: false, secrets: EMAIL_SECRETS },
  async (request) => {
    ...
    // Create or find Firebase Auth account
    let uid
    try {
      ...
      uid = (await admin.auth().createUser({ email, password: tempPw, displayName: `${displayFirst} ${displayLast}`.trim(), emailVerified: false })).uid
    }

    // Write Firestore profile FIRST so it exists when teacher logs in
    ...
    if (oldId && oldId !== uid) {
      try { await db.collection('users').doc(oldId).delete() } catch {}
    }

    // Generate Firebase password reset link
    // IMPORTANT: we extract the oobCode and build a DIRECT link to our app
    // so the teacher lands on our page with the code, not Firebase's hosted page
    const rawLink = await admin.auth().generatePasswordResetLink(email, {
      url: 'https://yrshifts.web.app/app',
    })

    // Extract oobCode and build direct app URL
    let appLink = rawLink  // fallback
    try {
      const parsed  = new URL(rawLink)
      const oobCode = parsed.searchParams.get('oobCode')
      const apiKey  = parsed.searchParams.get('apiKey')
      if (oobCode) {
        appLink = `https://yrshifts.web.app/app?mode=resetPassword&oobCode=${oobCode}&apiKey=${apiKey}&lang=en`
      }
    } catch (e) { console.warn('Could not parse reset link:', e.message) }

    // Send invite email with the direct app link
    const transport = makeTransport()
    if (transport) {
      try {
        await transport.sendMail({
          ...
          html: `<!DOCTYPE html><html><body style="font-family:sans-serif;background:#0F1117;color:#E8ECF4;padding:24px;margin:0">
            <div style="max-width:520px;margin:0 auto">
              ...
              <div style="background:#1C2030;border-radius:0 0 12px 12px;padding:24px">
                ...
                <p style="color:#5C6380;font-size:12px;margin:0">This link expires in 1 hour. If it doesn't work, copy and paste this into your browser:<br><span style="color:#4EA8D6;word-break:break-all">${appLink}</span></p>
              </div>
              <p style="color:#5C6380;font-size:11px;text-align:center;margin-top:16px">Young Rembrandts · ShiftHub</p>
            </div></body></html>`,
        })
      ...
      console.warn('SMTP not configured — invite link:', appLink)
    }
...
function parseShiftTime(t) {
  ...
  if (!m) return -1
  let h = parseInt(m[1])
  const mins = parseInt(m[2]), period = m[3].toUpperCase()
...
function buildReminderBody(user, shift, type) {
  ...
  if (type === '24h') return `Hi ${name},\n\nReminder: you have a shift TOMORROW.\n\n  📅 ${shift.date}\n  🕐 ${shift.start} – ${shift.end}\n  📚 ${shift.title || 'Shift'}${addr}${note}\n\nhttps://yrshifts.web.app/app`
...
function buildBody(n) {
  switch (n.type) {
    case 'shift_assigned':  return `Hi ${n.recipientName || 'there'},\n\nYou have a new shift:\n\n  📅 ${n.shiftDate}\n  🕐 ${n.shiftStart} – ${n.shiftEnd}\n  📚 ${n.shiftTitle || 'Shift'}\n\nPlease confirm or reject in the app.\n\nhttps://yrshifts.web.app/app`
    case 'shift_confirmed': return `${n.actorName} confirmed their shift on ${n.shiftDate} at ${n.shiftStart}.`
    case 'shift_rejected':  return `${n.actorName} rejected their shift on ${n.shiftDate}.\n\nLog in to reassign:\nhttps://yrshifts.web.app/admin`
    case 'shift_claimed':   return `${n.actorName} claimed the open shift on ${n.shiftDate} at ${n.shiftStart}.`
    default:                return n.message || 'You have a new ShiftHub notification.'
  }
}
